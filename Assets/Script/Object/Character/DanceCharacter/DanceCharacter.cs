...
public class DanceCharacter : MBehavior
{
	...
	[SerializeField] SpriteRenderer fakeShadow;
...
//				m_stateMachine.State = State.Disappear;
		...
		m_stateMachine.AddExit (State.BackToNormal, delegate() {
	...
	{
		return ((transform.position - m_agent.destination).magnitude <= m_agent.stoppingDistance) &&
	...
	[SerializeField] Material umberllaMaterial;
...
//		}
		...
		if (setMaterial) {
			setMaterial = false;
			dancerCharacter.SetMaterial (skinMaterial, umberllaMaterial);
		}
	}
...
//		M_Event.RegisterAll (OnEvent);
		M_Event.RegisterEvent (LogicEvents.ToDark, OnEvent);
		M_Event.RegisterEvent (LogicEvents.ToModern, OnEvent);
		M_Event.RegisterEvent (endEvent, OnEvent);
	...
	{
		base.MOnDisable ();
		M_Event.UnregisterEvent (LogicEvents.ToDark, OnEvent);
		M_Event.UnregisterEvent (LogicEvents.ToModern, OnEvent);
		M_Event.UnregisterEvent (endEvent, OnEvent);
