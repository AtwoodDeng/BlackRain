...
public class GirlAcrossStreet : TalkableCharacter {
	...
	[SerializeField] LogicManager.GameState prepareState = LogicManager.GameState.WalkIntoPeople;
	[SerializeField] FilmController endFilm;
	[SerializeField] Transform MCEndingPosition;
//	[SerializeField] LogicEvents forceGirlLeaveEvent = LogicEvents.ForceGirlLeave;
	[SerializeField] float girlSpeed;
	[System.Serializable]
	...
	void InitStateMachine()
	{
		...
		//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetEndTalkGirl, GirlState.Prepare, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelOneHalf, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelOne, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelTwoHalf, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelTwo, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelThreeHalf, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
//		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetLevelThree, GirlState.WaitForPlayer, GirlState.WalkWithUmbrella);
		m_stateMachine.BlindStateChangeEvent (LogicEvents.AcrossStreetGirlLeave, GirlState.GotoDisappear, GirlState.Disappear);
		...
		m_stateMachine.AddUpdate (GirlState.Prepare, delegate() {
			if ( LogicManager.Instance.State == LogicManager.GameState.AcrossStreetBeginLevel )
				m_stateMachine.State = GirlState.WalkWithUmbrella;	
		});

		m_stateMachine.AddExit (GirlState.Prepare, delegate() {
//			NextTarget();	
		});
...
//				NextTarget();
		...
		m_stateMachine.AddUpdate (GirlState.WalkWithUmbrella, delegate() {
			...
			if ( CheckTarget())
			{
				if ( LogicManager.Instance.State >= LogicManager.GameState.AcrossStreetThreeAndChangeMusic )
				{
					m_stateMachine.State = GirlState.WaitAtFinal;
				}else{
...
//			NextTarget();
		...
		m_stateMachine.AddUpdate (GirlState.WaitAtFinal, delegate() {
//			if ( m_IsPlayerIn )
//			{
//				m_stateMachine.State = GirlState.GotoDisappear;	
//			}
		});

		m_stateMachine.AddEnter (GirlState.GotoDisappear, delegate {
			endFilm.Work();
			Vector3 opposite = MainCharacter.Instance.transform.position - MCEndingPosition.position;
			Vector3 toLeave = opposite + transform.position;
			transform.DOMove( toLeave , 10f ).SetEase(Ease.Linear).SetDelay(7f);
			MainCharacter.Instance.transform.DOMove( MCEndingPosition.position , 10f ).SetEase(Ease.Linear).SetDelay(7f);
			m_agent.enabled = false;
		});

		m_stateMachine.AddExit (GirlState.GotoDisappear, delegate {
			MainCharacter.Instance.transform.DOKill();
			MainCharacter.Instance.transform.position = MCEndingPosition.position;	
		});
...
//		M_Event.UnregisterEvent (LogicEvents.PlayMusic, OnEvent);
	...
	void OnEvent(LogicArg arg)
	{
		...
		if (arg.type == LogicEvents.AcrossStreetEnterDance) {
			SetTarget (0);
			m_stateMachine.State = GirlState.WalkWithUmbrella;
		} else if (arg.type == LogicEvents.AcrossStreetLevelOneHalf) {
			SetTarget (1);
			m_stateMachine.State = GirlState.WalkWithUmbrella;

		} else if (arg.type == LogicEvents.AcrossStreetLevelOne) {
			SetTarget (2);
			m_stateMachine.State = GirlState.WalkWithUmbrella;
		} else if (arg.type == LogicEvents.AcrossStreetLevelTwo) {
			SetTarget (3);
			m_stateMachine.State = GirlState.WalkWithUmbrella;
		} else if (arg.type == LogicEvents.AcrossStreetLevelThree) {
			SetTarget (4);
			m_stateMachine.State = GirlState.WalkWithUmbrella;
		} 
		...
		if ( arg.type == LogicEvents.MusicBeat )
		{
			...
			if ( ( count % 4 == 1 ) && m_IsPlayerIn && m_stateMachine.State == GirlState.WaitAtFinal) {
				m_stateMachine.State = GirlState.GotoDisappear;
			}
		}
	}
...
//	}
	...
	void OnGUI()
	{
		if (m_stateMachine.State != GirlState.Init && m_stateMachine.State != GirlState.Disappear) {
			GUILayout.Label ("");
			GUILayout.Label ("GirlAcrossStreet " + m_stateMachine.State);
