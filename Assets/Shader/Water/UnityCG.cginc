...
// Shadow caster pass helpers
	...
	#define TRANSFER_GEOM_SHADOW_CASTER(o, vert) \
		o.vec = mul( unity_ObjectToWorld, vert ).xyz - _LightPositionRange.xyz; \
	...
	#define SHADOW_CASTER_FRAGMENT(i) return EncodeFloatRGBA( min(length(i.vec) * _LightPositionRange.w, 0.999) );
		...
		#define TRANSFER_GEOM_SHADOW_CASTER(o, vert) \
			o.pos = mul(UNITY_MATRIX_MVP, vert); \
			o.pos.z += unity_LightShadowBias.x; \
			clamped = max(o.pos.z, o.pos.w*UNITY_NEAR_CLIP_VALUE); \
			o.pos.z = lerp(o.pos.z, clamped, unity_LightShadowBias.y); \
			o.hpos = o.pos; \
			outStream.Append( o );
	#else
...
#define TRANSFER_GEOM_SHADOW_COLLECTOR(o, vert)	\
	o.pos = mul(UNITY_MATRIX_MVP, vert); \
	wpos = mul(unity_ObjectToWorld, vert); \
	o._WorldPosViewZ.xyz = wpos; \
	o._WorldPosViewZ.w = -mul( UNITY_MATRIX_MV, vert ).z; \
	o._ShadowCoord0 = mul(unity_WorldToShadow[0], wpos).xyz; \
	o._ShadowCoord1 = mul(unity_WorldToShadow[1], wpos).xyz; \
	o._ShadowCoord2 = mul(unity_WorldToShadow[2], wpos).xyz; \
	o._ShadowCoord3 = mul(unity_WorldToShadow[3], wpos).xyz; \
